...
const ThreatDetailsModal = ({ isOpen, onClose, threat }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal Content */}
      <div ref={modalRef} className="relative w-full max-w-3xl bg-[#020617] border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">

        {/* Background Grid */}
        <div className="absolute inset-0 pointer-events-none z-0">
          <div className="absolute top-0 right-0 w-full h-64 bg-rose-900/10 blur-[80px]"></div>
        ...
        <div className="p-6 border-b border-slate-800/60 flex justify-between items-start bg-[#0f172a]/80 backdrop-blur-md relative z-10 shrink-0">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-xl flex items-center justify-center shadow-lg border border-rose-500/20 bg-slate-900/50 text-rose-400">
              <ShieldAlert size={28} />
            </div>
            <div>
              <div className="flex items-center gap-3">
                <h2 className="text-xl font-bold text-white tracking-tight">Threat Details</h2>
                <span className="px-2 py-0.5 bg-slate-800 rounded text-[10px] font-mono text-slate-400 border border-slate-700">ID: #{threat.id}</span>
              </div>
              <p className="text-rose-400 text-xs font-bold uppercase tracking-wider mt-1 flex items-center gap-2">
        ...
        {/* Body */}
        <div className="p-8 overflow-y-auto custom-scrollbar relative z-10 flex-1 bg-[#020617]/50 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="modal-anim-item space-y-6">
              <div>
                ...
                <p className="text-white text-lg font-bold">{threat.name}</p>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 block">Current Status</label>
                <span className={`px-3 py-1.5 rounded-md text-xs font-bold uppercase tracking-wide inline-block border ${threat.status === 'Active' ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' :
                    threat.status === 'Investigating' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-mono font-bold text-white">{threat.confidence}%</span>
                </div>
              </div>
            </div>

            <div className="modal-anim-item bg-slate-900/50 rounded-xl border border-slate-800 p-5 space-y-4">
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-800 pb-2 mb-2">Technical Indicators</h4>
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-cyan-400 font-mono">{threat.ip}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Detected At</span>
                <span className="text-slate-300 font-mono text-xs">{threat.detected}</span>
              </div>
            </div>
          </div>
          ...
          <div className="modal-anim-item">
            ...
            <div className="bg-black/40 rounded-xl border border-slate-800 p-4 overflow-hidden relative group">
        ...
        <div className="p-6 border-t border-slate-800/60 bg-[#0f172a]/80 backdrop-blur-md flex justify-end gap-4 shrink-0 relative z-10">
          <button onClick={onClose} className="px-6 py-2.5 text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl transition-colors border border-transparent hover:border-slate-700">
            Close
          </button>
  ...
  return (
    <div className="flex min-h-screen bg-[#0f172a] text-slate-300 font-sans">
      ...
      <main ref={containerRef} className="flex-1 p-8 overflow-y-auto h-screen relative">
        ...
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-rose-900/10 rounded-full blur-[120px] pointer-events-none"></div>
        ...
        <div className="threat-header mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <p className="text-slate-400 text-sm">Real-time threat monitoring and automated response.</p>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2.5 rounded-lg border border-slate-700 text-slate-400 hover:text-white hover:bg-slate-800 transition shadow-md"
              title="Refresh Data"
            >
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-slate-700 text-slate-400 text-sm font-bold hover:bg-slate-800 hover:text-white transition-all shadow-sm uppercase tracking-wide"
              >
                <X size={16} /> Reset
              </button>
            </div>
        ...
        {/* Filters Area */}
        {showFilters && (
          <div className="threat-filter-bar bg-[#1e293b]/50 backdrop-blur-sm p-6 rounded-xl border border-slate-800 mb-8 grid grid-cols-1 md:grid-cols-4 gap-4 transition-all shadow-lg relative z-10">
            <div className="relative group">
              ...
              <input
                ...
                className="w-full bg-[#0f172a] border border-slate-700 text-white text-sm rounded-lg pl-10 pr-4 py-2.5 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none transition placeholder:text-slate-600"
              />
            </div>
            <select
              value={severityFilter}
              onChange={(e) => { setSeverityFilter(e.target.value); setPage(1); }}
              className="bg-[#0f172a] border border-slate-700 text-slate-300 text-sm rounded-lg px-4 py-2.5 outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all"
            >
        ...
        {/* Data Table */}
        <div className="threat-table-container bg-[#1e293b]/40 backdrop-blur-xl rounded-xl border border-slate-800 shadow-2xl overflow-hidden flex flex-col min-h-[500px] relative z-10">
          {/* Top Gradient Line */}
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-800/60 bg-[#0f172a]/80 text-slate-400 text-[11px] uppercase font-bold tracking-widest">
              ...
              <tbody className="divide-y divide-slate-800/50 bg-[#0b1120]/30">
                {loading && threats.length === 0 ? (
                  ...
                  pageItems.map((threat) => (
                    <tr key={threat.id} className="threat-row group hover:bg-slate-800/30 transition-colors border-l-2 border-l-transparent hover:border-l-cyan-500">
                      ...
                      <td className="p-5 text-slate-400 text-sm">
                        <span className="bg-slate-900 border border-slate-700 px-2 py-0.5 rounded text-xs font-mono">{threat.type}</span>
                      </td>
                      <td className="p-5">
                      ...
                      <td className="p-5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-300 font-mono font-bold">{threat.confidence}%</span>
                        </div>
                      </td>
                      <td className="p-5 text-slate-400 font-mono text-xs text-cyan-500/80">{threat.ip}</td>
                      <td className="p-5 text-slate-400 text-xs whitespace-nowrap font-mono">{threat.detected}</td>
                      ...
                      <td className="p-5 text-right relative">
                        <div className="relative inline-block text-left">
                          ...
                          {activeMenuId === threat.id && (
                            <div className="absolute right-0 mt-2 w-56 bg-[#020617] border border-slate-700 rounded-xl shadow-2xl z-50 overflow-hidden ring-1 ring-white/10 animate-in fade-in zoom-in-95 duration-100 origin-top-right">
                              <div className="py-1">
                                <DropdownItem
          ...
          {/* Pagination */}
          <div className="bg-[#0f172a]/80 backdrop-blur p-4 border-t border-slate-800 flex items-center justify-between mt-auto relative z-20">
            <span className="text-xs font-medium text-slate-400">
              Showing <span className="text-white">{Math.min((page - 1) * pageSize + 1, filtered.length)}</span> to{" "}
              <span className="text-white">{Math.min(page * pageSize, filtered.length)}</span> of{" "}
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-4 py-2 text-xs font-bold uppercase tracking-wide bg-slate-800 border border-slate-700 rounded-lg hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition shadow-sm text-slate-300 hover:text-white"
              >
                Previous
              </button>
              <button
