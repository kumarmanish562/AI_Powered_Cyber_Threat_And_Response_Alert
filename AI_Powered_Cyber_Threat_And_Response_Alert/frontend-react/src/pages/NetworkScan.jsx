...
const TopologyVisual = () => {
  ...
  return (
    <div ref={mapRef} className="w-full h-full bg-[#020617] relative overflow-hidden rounded-xl border border-slate-800">
      {/* Grid Background */}
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-10">
        <div className="relative">
          <div className="w-16 h-16 bg-blue-600 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(37,99,235,0.5)] border-4 border-[#020617] z-20 relative">
        ...
        <span className="mt-2 text-xs font-bold text-blue-400 bg-[#020617]/80 px-2 py-1 rounded border border-blue-500/30">GATEWAY</span>
      </div>
      ...
      {[
        ...
        <div key={i} className="node absolute flex flex-col items-center" style={{ left: node.x, top: node.y }}>
          <div className={`w-10 h-10 bg-slate-900 rounded-lg border border-${node.color}-500/30 flex items-center justify-center shadow-lg hover:scale-110 transition-transform cursor-pointer z-10`}>
            <node.icon size={18} className={`text-${node.color}-400`} />
          </div>
          ...
          <svg className="absolute top-1/2 left-1/2 w-[500px] h-[500px] -translate-x-1/2 -translate-y-1/2 -z-20 pointer-events-none opacity-20">
            <line x1="50%" y1="50%" x2={i % 2 === 0 ? "100%" : "0%"} y2="100%" stroke="white" strokeWidth="1" strokeDasharray="4 4" />
          </svg>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="flex min-h-screen bg-[#0f172a] text-slate-300 font-sans">
      ...
      <main className="flex-1 p-8 h-screen overflow-y-auto relative" ref={containerRef}>
        ...
        {/* Background Glow */}
        <div className="absolute top-0 right-0 w-[800px] h-[600px] bg-indigo-900/10 blur-[120px] pointer-events-none rounded-full"></div>
        ...
        <div className="scan-header flex flex-col md:flex-row justify-between items-end mb-8 relative z-10 gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl text-white font-bold tracking-tight">Network Scanner</h1>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs font-mono text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20 flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                Engine Ready
              </span>
              <p className="text-slate-400 text-sm">Discover assets, open ports, and map network topology.</p>
            </div>
          </div>

          <div className="scan-control-bar flex items-center gap-4 w-full md:w-auto bg-[#1e293b]/50 backdrop-blur-md p-2 rounded-xl border border-slate-800">
            {isScanning && (
              <div className="flex-1 md:w-64 px-4">
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-[10px] font-bold text-cyan-400 uppercase tracking-widest">Scanning Subnet</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 min-h-[500px]">
          ...
          <div className="scan-grid-item lg:col-span-2 relative h-full bg-[#020617] rounded-2xl border border-slate-800 shadow-2xl overflow-hidden group">

            {/* Map Overlay UI */}
            <div className="absolute top-4 left-4 z-20 bg-[#0f172a]/90 backdrop-blur-md p-4 rounded-xl border border-slate-800 shadow-xl w-64">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-blue-400 font-mono">192.168.1.0/24</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-slate-400">Latency</span>
                  <span className="text-purple-400 font-mono">~4ms</span>
                </div>
              </div>
            </div>

            <div className="absolute bottom-4 right-4 z-20 flex gap-2">
              <button className="p-2 bg-slate-800/80 backdrop-blur text-slate-400 hover:text-white rounded-lg border border-slate-700 transition-colors"><Activity size={16} /></button>
            </div>
          ...
          <div className="scan-grid-item flex flex-col h-full bg-[#020617] rounded-2xl border border-slate-800 shadow-2xl overflow-hidden">
